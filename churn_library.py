...
def classification_report_image(y_train,
    ...
    y_preds = [[y_train_preds_rf, y_test_preds_rf],
               [y_train_preds_lr, y_test_preds_lr]]
    models = ['Random Forest', 'Logistic Regression']

    for i, j in zip(y_preds, models):
        plt.rc('figure', figsize=(5, 5))
        plt.text(0.01, 1.25, str(j + 'Train'),
                 {'fontsize': 10}, fontproperties='monospace')
        plt.text(0.01, 0.05, str(classification_report(y_train, i[0])), {
                 'fontsize': 10}, fontproperties='monospace')  # approach improved by OP -> monospace!
        plt.text(0.01, 0.6, str(j + 'Test'),
                 {'fontsize': 10}, fontproperties='monospace')
        plt.text(0.01, 0.7, str(classification_report(y_test, i[1])), {
                 'fontsize': 10}, fontproperties='monospace')  # approach improved by OP -> monospace!
        plt.axis('off')
...
if __name__ == "__main__":
    ...
    train_models(
        features_train,
        features_test,
        label_train.values.ravel(),
        label_test.values.ravel())
